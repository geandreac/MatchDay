...
export default function Historico() {
  const [bookings, setBookings] = useState<HistoricBooking[]>([]);
  ...
  const statusLabel: Record<string, string> = {
    PENDING: "Pendente", CONFIRMED: "Confirmado", CANCELLED: "Cancelado", COMPLETED: "Concluído", REFUNDED: "Reembolsado",
  };
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-4 py-12">
          ...
          <p className="text-sm text-text-3">Nenhum histórico de reservas.</p>
          <p className="text-xs text-text-3/60">Suas reservas anteriores aparecerão aqui.</p>
        </div>
      ) : (
            ...
            <div key={b.id} className="card p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-text-3 mt-0.5">{new Date(b.date).toLocaleDateString("pt-BR")} • {String(b.startHour).padStart(2, "0")}h</p>
                </div>
